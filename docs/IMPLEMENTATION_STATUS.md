...
| 17.2 | Delivery rules (quiet hours, opt-out, digest) | ✅ | **Opt-out** (job creation): checks `NotificationPreference.enabled=false` and skips job creation; **Severity threshold** (job creation): filters events below `preference.severityThreshold`; **Quiet hours** (consumer): `NotificationKafkaConsumer` checks quiet hours window, reschedules jobs to end of quiet period with `status=SCHEDULED`; **Digest batching** (scheduler): `NotificationDigestScheduler` runs hourly via `@Scheduled`, queries `status=SCHEDULED AND scheduledFor<=now`, groups by recipient+channel, sends single summary message; V23 migration adds compound index `(status, scheduled_for)` | ✅ | Full test coverage planned: opt-out preferences, severity filtering, digest scheduling, quiet hours rescheduling, digest batch processing |
